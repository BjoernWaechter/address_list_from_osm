...
class TestRawAddress:
    ...
    def test_get_raw_address_as_polygon(self, test_context):
        ...
        assert isinstance(
            df_result.filter("id = 'N4680804790'").select("address_point").collect()[0].asDict()["address_point"],
            Point
        )

        assert isinstance(
            df_result.filter("id = 'W275123433'").select("address_point").collect()[0].asDict()["address_point"],
            Polygon
        )
